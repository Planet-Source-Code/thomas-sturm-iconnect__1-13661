...
Function Close_com()
    ...
    Close_com = CloseHandle(ComNum)
End Function
Function Init_Com(ComNumber, Comsettings As String) As Boolean
On Error GoTo handelinitcom
    ...
    If ComNum = -1 Then
        'we should notify the user here - alas, no one is listening :-)
...
handelpurecom:
    Exit Function
End Function
Function Write_Com(COMString As String) As Integer
On Error GoTo handelwritelpt
    Dim RetBytes As Long, LenVal As Long
    Dim retval As Long
    
    If Len(COMString) > 255 Then
        Write_Com Left$(COMString, 255)
        Write_Com Right$(COMString, Len(COMString) - 255)
        Exit Function
    End If
